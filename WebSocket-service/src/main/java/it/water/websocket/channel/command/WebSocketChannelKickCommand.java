...
@FrameworkComponent(services = {WebSocketChannelCommand.class, WebSocketChannelRemoteCommand.class}, properties = {
        WebSocketChannelConstants.COMMAND_FILTER + "=" + WebSocketChannelCommandType.KICK_USER_COMMAND
})
public class WebSocketChannelKickCommand extends WebSocketChannelAbstractCommand implements WebSocketChannelRemoteCommand {
    ...
    public void execute(WebSocketMessage message, String channelId, WebSocketChannelManager channelManager) {
        ...
        Optional<WebSocketUserInfo> kickerUserInfoOptional = channel.getPartecipantsInfo().stream().filter(userInfo -> userInfo.getUsername().equals(senderUsername)).findAny();
